...
const LENGTHS: { value: CopyLength; label: string; desc: string; sub: string }[] = [
  ...
  { value: 'medium', label: 'Medio',   desc: '125–300 palabras',   sub: 'Carruseles · Producto · Historias' },
  { value: 'long',   label: 'Extenso', desc: '300+ palabras',      sub: 'Storytelling · Tutoriales · Casos' },
];
...
function CopyCard({ option, onSave, isSaving }: { option: CopyOption; onSave: () => void; isSaving: boolean }) {
    ...
    <div className="rounded-2xl border border-white/8 bg-white/[0.03] overflow-hidden hover:border-[#FF5C2B]/30 transition-all group backdrop-blur-sm">
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/5 bg-white/[0.02]">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-white/30">{option.toneLabel}</span>
        </div>
      ...
      <div className="p-4">
        <p className="text-[13px] leading-relaxed text-white/80 whitespace-pre-wrap">{option.text}</p>
      </div>
    </div>
  );
}
        ...
        <div className="p-6 bg-white/[0.02] border border-white/5 rounded-3xl space-y-6">
          ...
          <div className="space-y-3">
            ...
            <textarea value={state.context} onChange={(e) => update({ context: e.target.value })}
              placeholder="Describe lo que necesitas comunicar..." rows={4}
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4">
              {state.options.map((opt) => (
                <CopyCard key={opt.id} option={opt} onSave={() => save(opt.id)} isSaving={state.isSaving} />
              ))}
            </div>
          </div>
        ) : (
